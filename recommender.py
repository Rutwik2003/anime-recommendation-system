...
try:
    # Get all anime ratings for current user
    user_ratings = user_anime_rating_matrix[user_anime_rating_matrix.index == current_user]
    
    # Get anime IDs where rating is > 0
    rated_anime = user_ratings.loc[:, user_ratings.gt(0).any()].columns.tolist()
    
    if not rated_anime:
        print(f"Error: User {current_user} has no rated anime.")
        sys.exit(1)
        
    # Sample one anime randomly
    leave_one_out_anime = sample(rated_anime, 1)[0]

except IndexError:
    print(f"Error: Could not find ratings for user {current_user}")
    sys.exit(1)
